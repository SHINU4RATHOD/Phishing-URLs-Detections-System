...
from core.dataset import HybridURLDataset
from core.loss import FocalLoss
from core.model import HybridGLUClassifier
from core.evaluator import EnhancedKPIEvaluator, SamsungDecisionEngine
from core.utils import CheckpointManager, ArtifactSaver, filter_inference_unsupported_test_urls, save_inference_url_screening_report


class HybridPhishingDetectionInference:
    """Main inference orchestrator supporting PyTorch checkpoints and ONNX Runtime for Hybrid GLU Fusion."""
    ...
    def load_datasets(self, norm_stats: Optional[Dict[str, Tuple[float, float]]], feature_cols: List[str]) -> Tuple[HybridURLDataset, HybridURLDataset, HybridURLDataset]:
    ...
    def create_model(self, vocab_size: Optional[int] = None) -> nn.Module:
        """Build base Hybrid GLU Fusion model."""
    ...
    def inference_from_checkpoint(self) -> bool:
        ...
        """
        print(f"\n{'='*80}")
        print("MINILM HYBRID GLU - INFERENCE MODE (CHECKPOINT RESUME)")
        print(f"{'='*80}")
        ...
        # Determine best model epoch and retrieve normalization stats and feature columns
        ...
        if best_model_dirs:
            ...
            best_model_dir = latest_checkpoint.parent
            best_model_epoch = 1
            print(f"[WARN] No best_model_epoch_* directory found, using checkpoint folder.")
            
        norm_stats_path = best_model_dir / "normalization_stats.json"
        feature_cols_path = best_model_dir / "feature_columns.json"
        ...
        if norm_stats_path.exists():
            with open(norm_stats_path, "r", encoding="utf-8") as f:
                ...
                # Parse back to tuples
                norm_stats = {k: (v[0], v[1]) for k, v in raw_stats.items()}
            print("[OK] Z-score normalization statistics loaded.")
        else:
            print("[WARN] Z-score statistics not found. Features will remain unnormalized.")
            
        if feature_cols_path.exists():
            with open(feature_cols_path, "r", encoding="utf-8") as f:
                feature_cols = json.load(f)
            print(f"[OK] Feature columns loaded ({len(feature_cols)} fields).")
        else:
            print("[WARN] Feature column list not found. Automatic heuristic columns will be detected.")
            # Auto detect from test set
            test_df = pd.read_csv(Config.TEST_CSV)
            feature_cols = HybridURLDataset.detect_feature_columns(test_df)
            
        # Load test dataset and filter unsupported URLs
        test_df = pd.read_csv(Config.TEST_CSV).reset_index(drop=True)
        ...
        print(f"[OK] Retained test dataset: {len(test_dataset):,} samples (Original: {len(test_df):,} samples)\n")
        ...
        model.eval()
        kpi_compliance = self._evaluate_test_set(
            model, test_loader, criterion, best_model_epoch, test_screening_report
        )
    ...
    def _evaluate_test_set(self, model: nn.Module, test_loader: DataLoader, criterion: nn.Module, 
                            best_epoch: int, test_screening_report: Dict[str, Any], save_root: Path = None) -> bool:
        ...
        print(f"[OK] Offline URL Category screening report saved.")
        
        print(f"\n{'='*60}")
        print("LOADING PRODUCTION HYBRID MODEL FOR TEST INFERENCE")
        print(f"{'='*60}")
        ...
        # Read deployment metadata for calibrated temperature, lambda, and threshold
        threshold = 0.5
        lambda_val = 0.5
        temp = 1.0
        ...
        if metadata_path.exists():
            try:
                ...
                if 'optimal_operating_point' in metadata:
                    oop = metadata['optimal_operating_point']
                    threshold = float(oop.get('base_threshold', threshold))
                    lambda_val = float(oop.get('lambda', lambda_val))
                    temp = float(oop.get('calibration_temp', temp))
                    print(f"[OK] Loaded calibrated hyperparameters from metadata:")
                    print(f"  Base Threshold: {threshold:.3f}")
                    print(f"  Lambda Boost:   {lambda_val:.2f}")
                    print(f"  Calibration T:  {temp:.3f}")
            except Exception as e:
                print(f"[WARN] Could not load optimal calibration parameters from metadata: {e}")
        ...
        if Config.CLASSIFICATION_LAYER_TYPE == "sigmoid":
            if len(test_logits.shape) > 1 and test_logits.shape[1] == 1:
                test_logits_squeezed = test_logits.squeeze(-1)
            else:
                test_logits_squeezed = test_logits
            calibrated_logits = test_logits_squeezed / temp
            log_odds = calibrated_logits
        else:
            calibrated_logits = test_logits / temp
            log_odds = calibrated_logits[:, 1] - calibrated_logits[:, 0]
        calibrated_probs = 1.0 / (1.0 + np.exp(-log_odds))
        
        # Run Samsung Decision Engine
    ...
    def onnx_inference(self, onnx_model_type: str = 'int8') -> bool:
        """
        ONNX Inference Mode: Load multi-input ONNX model and evaluate on test set.
        ...
        """
        print("\n" + "="*80)
        print("MINILM HYBRID GLU - ONNX INFERENCE MODE")
        print("="*80)
        print(f"Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        # Load Z-score stats and feature cols inside best_model folder
        norm_stats_path = best_model_dir / "normalization_stats.json"
        feature_cols_path = best_model_dir / "feature_columns.json"
        ...
        if norm_stats_path.exists():
            with open(norm_stats_path, "r", encoding="utf-8") as f:
                raw_stats = json.load(f)
                norm_stats = {k: (v[0], v[1]) for k, v in raw_stats.items()}
            print("[OK] Z-score statistics loaded.")
        if feature_cols_path.exists():
            with open(feature_cols_path, "r", encoding="utf-8") as f:
                feature_cols = json.load(f)
            print(f"[OK] Feature columns loaded ({len(feature_cols)} fields).")
        ...
        print(f"[OK] Retained test dataset: {len(test_dataset):,} samples\n")
        
        # Load calibrated parameters from metadata
        threshold = 0.5
        lambda_val = 0.5
        temp = 1.0
        ...
        if metadata_path.exists():
            try:
                ...
                if 'optimal_operating_point' in metadata:
                    ...
                    lambda_val = float(oop.get('lambda', lambda_val))
                    temp = float(oop.get('calibration_temp', temp))
                    print(f"[OK] Loaded calibrated operating point from metadata:")
                    print(f"  Base Threshold: {threshold:.3f}")
                    print(f"  Lambda Boost:   {lambda_val:.2f}")
                    print(f"  Calibration T:  {temp:.3f}")
            except Exception as e:
                print(f"[WARN] Could not load calibration parameters: {e}")
        ...
        sess_options.intra_op_num_threads = max(1, Config.NUM_WORKERS)
        ...
        try:
            ...
            # Map input names in the dynamic dynamic-batch ONNX graph
            input_names = [inp.name for inp in session.get_inputs()]
            output_names = [out.name for out in session.get_outputs()]
            print(f"[OK] ONNX inputs detected: {input_names}")
            print(f"[OK] ONNX outputs detected: {output_names}")
        except Exception as e:
            ort.set_default_logger_severity(1)
        ...
        for batch in tqdm(test_loader, desc="ONNX Inference"):
            ...
            # Formulate the triple dynamic inputs
            ort_inputs = {
                'input_ids': input_ids,
                'attention_mask': attention_mask,
                'heuristic_features': heuristic_features
        ...
        if Config.CLASSIFICATION_LAYER_TYPE == "sigmoid":
            if len(test_logits.shape) > 1 and test_logits.shape[1] == 1:
                ...
                test_logits_squeezed = test_logits
            calibrated_test_logits = test_logits_squeezed / temp
            log_odds = calibrated_test_logits
        else:
        ...
        save_inference_url_screening_report(test_screening_report, onnx_results_dir)
        ...
        # Print summary
        print(f"\n{'='*80}")
        print("ONNX INFERENCE RESULTS (HYBRID GLU FUSION)")
        print(f"{'='*80}")
...
def main() -> bool:
    ...
    parser = argparse.ArgumentParser(
        description="MiniLM Hybrid GLU Fusion URL Detection - Modular Inference Pipeline",
    ...
    try:
        if args.mode == 'inference':
            inference = HybridPhishingDetectionInference()

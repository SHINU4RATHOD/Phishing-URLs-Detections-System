...
class MLflowManager:
    ...
    Tailored specifically for the Hybrid GLU Fusion architecture.
    """
    ...
    def start_run(self, run_id: Optional[str] = None) -> Optional[str]:
        ...
        Args:
            run_id: Optional run ID to resume (vital for training continuity).
        ...
        try:
            # Resolve DagsHub integration first if specified
            if hasattr(Config, "MLFLOW_DAGSHUB_REPO") and Config.MLFLOW_DAGSHUB_REPO:
                try:
                    import dagshub
                    repo_owner, repo_name = Config.MLFLOW_DAGSHUB_REPO.split('/')
                    logger.info(f"[MLflow] Initializing DagsHub Client: {Config.MLFLOW_DAGSHUB_REPO}")
                    dagshub.init(repo_owner=repo_owner, repo_name=repo_name, mlflow=True)
                except ImportError:
                    logger.warning("[MLflow] 'dagshub' package not found. Falling back to basic HTTP authentication.")
                    if hasattr(Config, "MLFLOW_TRACKING_USERNAME") and Config.MLFLOW_TRACKING_USERNAME:
                        os.environ["MLFLOW_TRACKING_USERNAME"] = Config.MLFLOW_TRACKING_USERNAME
                    if hasattr(Config, "MLFLOW_TRACKING_PASSWORD") and Config.MLFLOW_TRACKING_PASSWORD:
                        os.environ["MLFLOW_TRACKING_PASSWORD"] = Config.MLFLOW_TRACKING_PASSWORD
                except Exception as e:
                    logger.error(f"[MLflow] Failed to initialize DagsHub client: {e}")
            else:
    ...
    def _log_initial_configurations(self):
        """Helper to log hybrid model hyperparameters and metadata tags."""
        if not self.enabled or self.mlflow is None:
            return
        ...
        try:
            ...
            params = {
                ...
                "HEURISTIC_MLP_HIDDEN": Config.HEURISTIC_MLP_HIDDEN,
                "HEURISTIC_MLP_OUTPUT": Config.HEURISTIC_MLP_OUTPUT,
                "GLU_HIDDEN": Config.GLU_HIDDEN,
            ...
            tags = {
                "project": "PhishURL-Detection-Hybrid",
                "architecture": "MiniLM-L12 + HeuristicMLP + GLUGate + LoRA",
                "framework": "PyTorch 2.x",
                "scale": "Production 50M URL ready",
                "mlops_tool": "MLflow",
                "environment": "Sanity test" if "test" in Config.MLFLOW_EXPERIMENT_NAME.lower() else "Production"
            }

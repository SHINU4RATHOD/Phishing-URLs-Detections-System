...
class URLSplitterAndReporter:
    ...
    def _prepare_mode_row(self, row_dict: Dict, mode: str) -> Dict:
        ...
        if mode == "raw_orig":
            ...
            # Extract all h_ and hF_ columns, plus base metadata for decision engine
            hybrid_row = {
                "input": row_dict.get("canonical_url", ""), 
                "label": label,
                "primary_category": row_dict.get("primary_category", "None"),
                "flags_count": row_dict.get("flags_count", 0),
        ...
        with pd.read_csv(
            ...
            for chunk in tqdm(reader, desc="Splitting Hybrid"):
                ...
                for _, row in shuffled.iterrows():
                    lab = int(row["label"])
                    pc = str(row.get("primary_category", "") or "") or "UNKNOWN"

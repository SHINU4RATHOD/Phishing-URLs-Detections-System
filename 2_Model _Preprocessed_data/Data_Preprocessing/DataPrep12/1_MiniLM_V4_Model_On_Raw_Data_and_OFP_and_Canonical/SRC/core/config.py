...
class Config:
    ...
    DROPOUT: float = 0.15
    CLASSIFIER_DIMS: List[int] = [384, 512, 256, 128, 64]
    ...
    FOCAL_ALPHA: List[float] = [0.28, 0.72]
    ...
    MLFLOW_EXPERIMENT_NAME: str = "PhishURL-Detection-MiniLM"
    MLFLOW_RUN_NAME: str = "MiniLM-V4-Production-Pipeline"
    MLFLOW_REGISTER_MODEL: bool = True
    MLFLOW_MODEL_NAME: str = "PhishURL-MiniLM-Classifier"
    ...
    def load_from_yaml(cls, yaml_path: Optional[str] = None, create_dirs: bool = True, verbose: bool = True) -> None:
        ...
        if yaml_path is None:
            # Look in parent directories or default location
            possible_paths = [
                Path("4_config.yaml"),
        ...
        if yaml_path and not Path(yaml_path).exists():
            # Try to resolve relative to project root or script parent directory
        ...
        try:
            ...
            # Map the nested YAML structure to the flat class attributes
            ...
            if 'loss' in data:
                l = data['loss']
                cls.FOCAL_GAMMA_POS = l.get('focal_gamma_pos', cls.FOCAL_GAMMA_POS)
                cls.FOCAL_GAMMA_NEG = l.get('focal_gamma_neg', cls.FOCAL_GAMMA_NEG)
            ...
            if 'class_balancing' in data:
                ...
                cls.USE_STRATIFIED_KFOLD = cb.get('use_stratified_kfold', cls.USE_STRATIFIED_KFOLD)
                cls.KFOLD_SPLITS = cb.get('kfold_splits', cls.KFOLD_SPLITS)
    ...
    def setup_paths(cls, create_dirs: bool = True) -> None:
        ...
        # Helper to resolve relative paths against project root dynamically
        def resolve_path(p):
            if not p:
                return p
            path_obj = Path(p)
        ...
        if cls.SAVE_ROOT is None:
            cls.SAVE_ROOT = project_root / "RESULTS_&_MODELS/3_saved_models/MiniLM_raw_orig4"
        else:
    ...
    def setup_reproducibility(cls) -> None:
        """Set dynamic seeds across libraries."""
    ...
    def _manual_parse(cls, yaml_path: str) -> None:
        """Extremely simple manual fallback line-by-line parser for config values."""
        try:
            with open(yaml_path, 'r', encoding='utf-8') as f:
                lines = f.readlines()
            ...
            print(f"[WARN] Manual fallback configuration parser failed: {e}")

...
class SwiGLUGate(nn.Module):
    ...
    Mechanism:
        concat = [text_emb ; feat_emb]          # (batch, 384 + 192 = 576)
        gate   = SiLU(W_gate · concat + b_gate)   # (batch, hidden_dim = 384)
        value  = W_val · concat + b_val          # (batch, hidden_dim = 384)
        output = gate ⊙ value                     # (batch, hidden_dim = 384)
    """
    ...
    def forward(self, text_emb: torch.Tensor, feat_emb: torch.Tensor) -> torch.Tensor:
        concat = torch.cat([text_emb, feat_emb], dim=-1)  # (batch, 576)
        gate = self.silu(self.gate_proj(concat))            # (batch, 384)
        value = self.value_proj(concat)                     # (batch, 384)
        fused = gate * value                                  # (batch, 384)
        return self.layer_norm(fused)
...
class HybridGLUClassifier(nn.Module):
    ...
    Fusion:               [384; 192] → GLU/SwiGLU Gate → 384-dim
    ...
    def __init__(self, vocab_size: Optional[int] = None):
        ...
        if gating_type == "SWIGLU":
            print("[MODEL] Instantiating SwiGLUGate gating mechanism for fusion.")
            ...
            print("[MODEL] Instantiating standard GLUGate gating mechanism for fusion.")
...
class ModelCalibrator(nn.Module):
    ...
    def calibrate(self, valid_logits: torch.Tensor, valid_labels: torch.Tensor):
        if Config.CLASSIFICATION_LAYER_TYPE == "sigmoid":
            ...
            logits_squeezed = valid_logits.squeeze(-1) if len(valid_logits.shape) > 1 else valid_logits
            labels_float = valid_labels.float()
            
            def eval_fn():
                optimizer.zero_grad()
                loss = nll_criterion(self.forward(logits_squeezed), labels_float)
                loss.backward()
                return loss
        else:
